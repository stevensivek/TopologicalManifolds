...
- `boundaryless_interior`: if `X` is homeomorphic to `I.interior Y`, then `X`
...
theorem homeomorph_image_interior [HasInvarianceOfDomain E] {X Y : Type*}
  ...
  · rintro ⟨x, hxInt, hxy⟩
    ...
    have hψy : ψ y = (chartAt H x) x := by simp [ψ, ← hxy]
...
variable (I) in
theorem boundaryless_interior [HasInvarianceOfDomain E] {X Y : Type*}
    [TopologicalSpace X] [TopologicalSpace Y] [ChartedSpace H X] [ChartedSpace H Y]
